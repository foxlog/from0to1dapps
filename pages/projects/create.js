...
class ProjectCreate extends React.Component {
    ...
    async createProject() {
        ...
        if (minInvest <= 0) {
            return this.setState({errmsg: '项目最小金额必需大于0'});

        }
        if (maxInvest < minInvest) {
            return this.setState({errmsg: '项目最小投资金额必需小于最大金额'});
        }
        if (goal <= 0) {
            return this.setState({errmsg: '项目募资上限必需大于0'});
        }
        ...
        try {
            ...
            //创建项目
            const result = await ProjectList.methods.createProject(
                description, minInvestInWei, maxInvestInWei, goalInWei
            ...
            console.error(error);
            this.setState({errmsg: error.message || err.toString()});
